...
import click
import toml
from moseq2_pca.util import combine_new_config
...
@click.pass_context
def train_pca(ctx, input_dir, output_dir, output_file, **cli_args):
    ...
    if ctx.obj.get('config_file'):
        ...
        combine_new_config(ctx.obj.get('config_file'), config_data)
...
@click.pass_context
def apply_pca(ctx, input_dir, output_dir, output_file, **cli_args):
...
@click.option('--verbose', '-v', is_flag=True, help="Print sessions as they are being loaded.")
@click.pass_context
def compute_changepoints(ctx, input_dir, output_dir, output_file, **cli_args):
